...
We can see that bid rigging ("conluio") and overpricing ("superfaturamento") are the two most frequent fraud-related words in government publications ("fraudes" is just the plural form of "fraude"). Kinda cool to see it. You can also cluster the word embeddings to find groups of inter-related words; use t-SNE to reduce dimensionality to 2 so you can plot the embeddings on an XY plot; and try a bunch of other [fun ideas](https://www.kaggle.com/pierremegret/gensim-word2vec-tutorial).

Here I trained the word embeddings from scratch but you could also take pre-trained Brazilian Portuguese word2vec embeddings and use the Diário to fine-tune them. You could also tweak the parameters here, changing the window (10 here) and the number of dimensions (300). Whatever works best for the task you have at hand.

That's all for today! And remember, bureaucratese is bad writing - don't spend too long looking at those texts, lest you start emulating them. The best antidote I know is [William Zinsser](https://www.amazon.com/Writing-Well-Classic-Guide-Nonfiction/dp/0060891548).
